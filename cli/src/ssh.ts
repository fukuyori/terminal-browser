...
export async function openSshTunnel(
  ...
  const args = sshTunnelArgs(destination, hostArgs, socksPort, controlPath);
  ...
  if (!controlPath) process.once("exit", stopPersistent);
  if (controlPath) {
    ...
    try {
      await waitForSocks(socksPort, destination, 120_000, () => {
        ...
        if (child.exitCode !== null) return new Error(`ssh to ${destination} failed`);
        return null;
      });
    } catch (error) {
      stopPersistent();
