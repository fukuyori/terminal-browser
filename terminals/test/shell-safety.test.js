...
for (const [name, argv] of SHELLS) {
  test(`${name}: the harness does detect execution (a raw newline runs the command)`, posixOnly, () => {
    const file = marker();
  ...
  test(`${name}: a single line full of shell metacharacters and no newline never executes`, posixOnly, () => {
    const file = marker();
  ...
  test(`${name}: pressing Enter on the single-quoted payload executes nothing`, posixOnly, () => {
    const file = marker();
  ...
  if (name === "zsh") {
    test(`${name}: multi-line text wrapped in bracketed paste is inserted, not executed`, posixOnly, () => {
      const file = marker();
